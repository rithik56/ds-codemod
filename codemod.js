...
async function runJscodeshiftTransform(transform, files, flags, codemodFlags) {
  const paths = bases.flatMap(base =>
    exts.flatMap(ext => [
      path.resolve(__dirname, base, `${transform}/index.${ext}`),
      path.resolve(__dirname, base, `${transform}.${ext}`),
    ])
  );
...
yargs
  .command({
    ...
    describe: 'Applies a `@am92/react-design-system` to the specified paths',
    builder: (command) => {
      return command
        .positional('codemod', {
        ...
        .option('packageName', {
          ...
          default: '@am92/react-design-system',
          type: 'string',
        });
    },
    handler: run,
  })
  .scriptName('npx @am92-react-design-system/codemod')
