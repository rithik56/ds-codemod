export default function transformer(file, api, options) {
  ...
  root
    ...
    .filter(({ node }) => node.source.value.match(/^(@material-ui\/core|@am92\/material)\/Link/))
    .forEach(({ node }) => {
  ...
  root
    .find(j.ImportDeclaration)
    .filter(({ node }) => node.source.value.match(/^(@material-ui\/core|@am92\/material)\/?$/))
    .forEach(({ node }) => {
      node.specifiers.forEach((s) => {
        if (s.imported.name === "DsLink") {
          jsxLinkNames.push(s.local.name);
          node.source.value = node.source.value.replace('@material-ui/core', '@mui/material');
        }
      });
