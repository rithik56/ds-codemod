  ...
  root
    ...
    .forEach((path) => {
      path.node.specifiers.forEach((elementNode) => {
        if (
          (elementNode.type === 'ImportSpecifier' && elementNode.imported?.name === "DsTextField") ||
          elementNode.type === 'ImportDefaultSpecifier'
        ) {
          if (elementNode.imported?.name === "DsTextField") {
            elementNode.imported.name = "DsInput";
          }
          ...
          root.findJSXElements(elementNode.local.name).forEach((elementPath) => {
            ...
            if (formControlAttributeNodes.length > 0 || formLabelValue || formHelperTextValue) {
              const formControlIdentifier = j.jsxIdentifier("DsFormControl");
              const childrenOfFormControl = [];

              if (formLabelValue) {
                const formLabelIdentifier = j.jsxIdentifier("DsFormLabel");
              ...
              if (formHelperTextValue) {
                const formHelperTextIdentifier = j.jsxIdentifier("DsFormHelperText");
