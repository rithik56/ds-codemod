...
export default function transformer(fileInfo, api, options) {
  ...
  root
    ...
    .forEach((path) => {
      ...
      const subPackageImportMatch = importSource.match(/@am92\/lab\/(.*)/);
      if (subPackageImportMatch !== null) {
        ...
        if (xImportSpecifiers.length > 0) {
          const targetPackage = isPro ? '@mui/x-date-pickers-pro' : '@am92/react-design-system';
          path.replace(
